...
/**
 * Directions URL for a training session row (facility address when linked, else pool label).
 * @param {{ pool_location?: string, facilities?: { name?: string, address?: string } | null } | null | undefined} session
 * @returns {string | null}
 */
export function buildSessionDirectionsUrl(session) {
  if (!session) return null
  const facility = session.facilities
  if (facility && (facility.name || facility.address)) {
    return buildDirectionsUrl(facility)
  }
  return buildDirectionsUrlFromLabel(session.pool_location)
}
