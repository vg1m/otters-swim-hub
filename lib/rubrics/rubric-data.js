...
/** Admin squad form: selectable rubric checklists (pathway templates). */
export const RUBRIC_TEMPLATE_OPTIONS = RUBRIC_SQUAD_ORDER.map((slug) => ({
  value: slug,
  label: RUBRIC_DATA[slug]?.name ?? slug,
}))

/** Human label for a stored rubric_template_slug or pathway slug. */
export function rubricTemplateLabel(slug) {
  if (!slug) return null
  return RUBRIC_DATA[slug]?.name ?? slug
}
