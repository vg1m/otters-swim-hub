...
/**
 * Daily digest to admin: sessions past end time that still need a pay line recorded.
 * @param {{ to: string, pending: Array<{ sessionDate: string, coachName: string, amountKes: number }> }} params
 */
export async function sendCoachSessionPayAdminReminderEmail({ to, pending }) {
  try {
    const origin = emailPublicOrigin()
    const reviewUrl = `${origin}/admin/coaches#service`
    const count = pending.length
    const headline =
      count === 1
        ? '1 session needs a pay line recorded'
        : `${count} sessions need pay lines recorded`

    const listLimit = 20
    const shown = pending.slice(0, listLimit)
    const overflow = count > listLimit ? count - listLimit : 0

    const textLines = shown.map(
      (row) =>
        `  · ${row.sessionDate} — ${row.coachName} — KES ${Number(row.amountKes).toLocaleString()}`
    )
    const textBody = buildOttersEmailText({
      recipientName: 'Admin',
      headline,
      bodyLines: [
        'The following coached sessions have ended but do not have a pay line yet.',
        'Open Coach Management → Service & pay review to record each stub and notify the coach.',
        '',
        ...textLines,
    ...
    const listHtml = shown
      .map(
        (row) =>
          `<li style="margin:0 0 6px;font-size:14px;line-height:1.45;color:#374151;font-family:system-ui,sans-serif;">${escapeHtml(row.sessionDate)} · ${escapeHtml(row.coachName)} · <strong>KES ${escapeHtml(Number(row.amountKes).toLocaleString())}</strong></li>`
      )
      .join('')
    ...
    const html = buildOttersEmailHtml({
      ...
      bodyHtml: [
        emailParagraph(
          'These coached sessions have ended but do not have a pay line recorded yet. Record each stub in Coach Management so the coach receives their pay notification.'
        ),
        `<ul style="margin:12px 0 0;padding-left:20px;">${listHtml}</ul>`,
        overflow > 0
          ? emailParagraph(`… and ${overflow} more session${overflow === 1 ? '' : 's'}.`, {
              muted: true,
            })
          : '',
      ].join(''),
      ctaLabel: 'Record pay in Coach Management',
      ctaUrl: reviewUrl,
    })

    const result = await smtp2goSend({
      to,
      subject: `${headline} (Otters Kenya Academy of Swimming.)`,
      textBody,
      htmlBody: html,
    })
    ...
    return {
      success: result.ok,
      message: result.ok
        ? 'Coach pay admin reminder sent'
        : result.skipped
          ? 'Coach pay admin reminder skipped (SMTP2GO not configured)'
          : `Coach pay admin reminder failed: ${result.error}`,
      ...(!result.ok ? { error: result.error, skipped: !!result.skipped } : {}),
      ...(result.emailId ? { emailId: result.emailId } : {}),
    }
  } catch (error) {
    console.error('sendCoachSessionPayAdminReminderEmail error:', error)
    return { success: false, error: error instanceof Error ? error.message : String(error) }
  }
}
