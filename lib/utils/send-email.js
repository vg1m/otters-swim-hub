...
const EMAIL_SUPPORT_PLACEHOLDER_TEXT = '\n\nSupport: Otters.support@gmail.com\n'
const EMAIL_SUPPORT_PLACEHOLDER_HTML = `<p style="margin-top: 20px; margin-bottom: 0; font-size: 12px; color: #cbd5e1; line-height: 1.5;">Support: Otters.support@gmail.com</p>`
...
/**
 * Club announcement email to parents.
 */
export async function sendClubAnnouncementEmail({
  ...
  const safeTitle = escapeHtml(title || 'Club announcement')
  const safeBody = body ? escapeHtml(body) : ''
  const linkBlock = linkUrl
    ? `<p><a href="${escapeHtml(linkUrl)}" style="color:#0084d5;">${escapeHtml(linkUrl)}</a></p>`
    : ''
  const subject = `${title || 'Club announcement'} — Otters Swim Hub`

  const text = `Dear ${parentName || 'Parent'},

${title || 'The club has posted an announcement.'}
${body ? `\n${body}\n` : ''}
${linkUrl ? `\nLink: ${linkUrl}\n` : ''}
View in the app: ${dashboardUrl}
${EMAIL_SUPPORT_PLACEHOLDER_TEXT}Otters Kenya Academy of Swimming Limited
`
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    <h2 style="color: #0084d5;">${safeTitle}</h2>
    <p>Dear ${escapeHtml(parentName || 'Parent')},</p>
    ${safeBody ? `<p style="color: #475569;">${safeBody}</p>` : '<p>The club has posted a new announcement.</p>'}
    ${linkBlock}
    <p><a href="${dashboardUrl}" style="display:inline-block;background:#0084d5;color:#fff;padding:12px 24px;text-decoration:none;border-radius:6px;">View notifications</a></p>
    ${EMAIL_SUPPORT_PLACEHOLDER_HTML}
    ${EMAIL_COMPANY_GREY_HTML}
  </div>
</body>
...
/**
 * Coach broadcast email to parents in coach scope.
 */
export async function sendCoachBroadcastParentEmail({
  ...
  const dashboardUrl = `${emailPublicOrigin()}/dashboard/notifications`
  const safeTitle = escapeHtml(title || 'Message from your coach')
  const safeBody = body ? escapeHtml(body) : ''
  const linkBlock = linkUrl
    ? `<p><a href="${escapeHtml(linkUrl)}" style="color:#0084d5;">${escapeHtml(linkUrl)}</a></p>`
    : ''
  const subject = `${title || 'Message from your coach'} — Otters Swim Hub`
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    <h2 style="color: #0084d5;">${safeTitle}</h2>
    <p>Dear ${escapeHtml(parentName || 'Parent')},</p>
    <p style="color:#64748b;">Message from your coach</p>
...
/**
 * Admin reply to parent feedback (primary account email).
 */
export async function sendParentFeedbackResponseEmail({
  ...
  const safeSubject = escapeHtml(subject || 'Your feedback')
  const safeResponse = escapeHtml(adminResponse || '')
  const emailSubject = `Reply: ${subject || 'Your feedback'} — Otters Swim Hub`

  const text = `Dear ${parentName || 'Parent'},

We have responded to your feedback regarding "${subject || 'your message'}".

${adminResponse || ''}

View in the app: ${feedbackUrl}
${EMAIL_SUPPORT_PLACEHOLDER_TEXT}Otters Kenya Academy of Swimming Limited
`
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    <h2 style="color: #0084d5;">Reply to your feedback</h2>
    <p>Dear ${escapeHtml(parentName || 'Parent')},</p>
    <p style="color:#64748b;">Re: ${safeSubject}</p>
    <p style="white-space:pre-wrap;">${safeResponse}</p>
    <p><a href="${feedbackUrl}" style="display:inline-block;background:#0084d5;color:#fff;padding:12px 24px;text-decoration:none;border-radius:6px;">View feedback</a></p>
    ${EMAIL_SUPPORT_PLACEHOLDER_HTML}
    ${EMAIL_COMPANY_GREY_HTML}
  </div>
</body>
