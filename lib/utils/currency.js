...
import { isCurrentlyInEarlyBirdWindow } from '@/lib/billing/early-bird-window'
...
export const EARLY_BIRD_DISCOUNT = 2000     // deducted from monthly fee if paid 25th–3rd (club TZ)
...
export function formatSessionsPerWeekLabel(sessionsPerWeek) {
  if (sessionsPerWeek == null || sessionsPerWeek === '') return 'N/A'
...
/** True when today is in the 25th→3rd early-bird payment window (club TZ). */
export function isEarlyBirdEligible() {
  return isCurrentlyInEarlyBirdWindow(new Date())
}
