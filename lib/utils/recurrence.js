...
import {
  ...
  startOfDay, startOfWeek, endOfDay,
} from 'date-fns'

/** Parse DATE column (yyyy-MM-dd) in local time — avoids parseISO UTC day shift. */
export function parseSessionDateOnly(value) {
  if (value == null || value === '') return startOfDay(new Date())
...
function toDateStringFromValue(value) {
  if (!value) return ''
  if (typeof value === 'string') return value.slice(0, 10)
  return toDateString(parseSessionDateOnly(value))
}

function isRecurringSession(session) {
  return session?.is_recurring === true || session?.is_recurring === 'true'
}

/** Inclusive yyyy-MM-dd range compare (avoids startOfDay/endOfDay edge bugs). */
function isDateStrInRange(dateStr, startStr, endStr) {
  if (!dateStr || !startStr || !endStr) return false
  return dateStr >= startStr && dateStr <= endStr
}

function normalizeRecurrencePattern(raw) {
  let pattern =
    typeof raw === 'string'
      ? (() => {
          try {
  ...
  if (pattern.type) {
    pattern = { ...pattern, type: String(pattern.type).toLowerCase() }
  } else {
    pattern = { type: 'weekly' }
  }
  ...
  const known = new Set([
    ...
    'weekly_on_day',
    'monthly_on_week_day',
    'monthly_on_first_last',
    'annually',
    'custom',
  ])
  if (!known.has(pattern.type)) {
    return { type: 'weekly' }
  }
  return pattern
}
...
/**
 * Build a map of session_id|occurrence_date -> exception row (with optional squads).
 * @param {Array} exceptions - Rows from training_session_exceptions (+ nested squads)
 */
export function buildExceptionMap(exceptions) {
  const map = new Map()
  for (const ex of exceptions || []) {
    const key = `${ex.session_id}|${ex.occurrence_date}`
    map.set(key, ex)
  }
  return map
}

function applyExceptionToOccurrence(session, occurrenceDateStr, anchorDateStr, exception) {
  const base = {
    ...session,
    session_date: occurrenceDateStr,
    occurrence_date: occurrenceDateStr,
    series_anchor_date: anchorDateStr,
    is_series_occurrence: true,
  }
  if (exception?.status === 'cancelled') return null
  if (exception?.status !== 'override') return base

  const squadRows = exception.training_session_exception_squads
  const merged = {
    ...base,
    start_time: exception.start_time ?? session.start_time,
    end_time: exception.end_time ?? session.end_time,
    coach_id: exception.coach_id !== undefined && exception.coach_id !== null
      ? exception.coach_id
      : session.coach_id,
    facility_id: exception.facility_id !== undefined && exception.facility_id !== null
      ? exception.facility_id
      : session.facility_id,
    pool_location: exception.pool_location ?? session.pool_location,
  }
  if (squadRows?.length) {
    merged.training_session_squads = squadRows.map((row) => ({
 ...
 * @param {Map|string} [exceptionMapOrList] - Map from buildExceptionMap or exception rows
 * @returns {Array} Flat list of occurrence objects
 */
export function expandRecurringSessions(sessions, windowStart, windowEnd, exceptionMapOrList) {
  const results = []
  const winStartStr = toDateStringFromValue(windowStart) || toDateString(startOfDay(windowStart))
  const winEndStr = toDateStringFromValue(windowEnd) || toDateString(startOfDay(windowEnd))
  const winStart = startOfDay(windowStart)
  const winEnd = endOfDay(windowEnd)
  const MAX_ITER = 2000
  const exceptionMap =
    exceptionMapOrList instanceof Map
      ? exceptionMapOrList
      : buildExceptionMap(exceptionMapOrList)

  for (const session of sessions) {
    const origin = parseSessionDateOnly(session.session_date)
    const anchorDateStr = toDateString(origin)

    if (!isRecurringSession(session)) {
      if (isDateStrInRange(anchorDateStr, winStartStr, winEndStr)) {
        const ex = exceptionMap.get(`${session.id}|${anchorDateStr}`)
        const merged = applyExceptionToOccurrence(
          session,
          anchorDateStr,
          anchorDateStr,
          ex
        ...
        if (merged) {
          results.push({ ...merged, is_series_occurrence: false })
        }
      }
      continue
    }

    const pattern = normalizeRecurrencePattern(session.recurrence_pattern)

    let hardEndStr = toDateStringFromValue(session.recurrence_end_date)
    if (hardEndStr && hardEndStr <= anchorDateStr) {
      hardEndStr = ''
    }
    if (hardEndStr && hardEndStr < winStartStr) {
      continue
    }

    const upperBoundStr = hardEndStr && hardEndStr < winEndStr ? hardEndStr : winEndStr
    const upperBound = hardEndStr
      ? endOfDay(parseSessionDateOnly(hardEndStr))
      : winEnd

    let pushed = 0

    const pushIfInWindow = (candidate) => {
      const occurrenceDateStr = toDateString(candidate)
      if (occurrenceDateStr < anchorDateStr) return
      if (occurrenceDateStr > upperBoundStr) return
      if (!isDateStrInRange(occurrenceDateStr, winStartStr, winEndStr)) return

      const ex = exceptionMap.get(`${session.id}|${occurrenceDateStr}`)
      const merged = applyExceptionToOccurrence(
        session,
        occurrenceDateStr,
        anchorDateStr,
    ...
    if (pushed === 0 && isDateStrInRange(anchorDateStr, winStartStr, winEndStr)) {
      const ex = exceptionMap.get(`${session.id}|${anchorDateStr}`)
      const merged = applyExceptionToOccurrence(
        session,
        anchorDateStr,
