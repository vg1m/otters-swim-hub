...
export default function DashboardNextSessionStrip({ session, onOpenSchedule }) {
  ...
  return (
    <button
      ...
      aria-label={`Next session ${formatDate(session.session_date)}. Tap to view schedule.`}
    >
      <Card
        padding="sm"
      ...
      >
        ...
        <span className="text-xs text-primary mt-2 inline-block">View full schedule →</span>
      </Card>
    </button>
  )
}
