...
/**
 * Parent-facing attendance calendar.
 *
 * Driven by two inputs:
 *   - `scheduledSessions`: canonical list of this swimmer's expanded sessions
 *     (recurring bases already expanded into concrete occurrences).
 *   - `attendance`: attendance rows for the swimmer. Presence of a row on a
 *     given session_date means the swimmer was checked in; absence on a past
 *     scheduled date is counted as missed.
 */
export default function AttendanceCalendarModal({
  ...
  // Index scheduled sessions by yyyy-MM-dd so lookups are O(1) per cell.
  ...
  // Index attendance rows by yyyy-MM-dd via the joined training_sessions row.
  ...
  const monthStats = useMemo(() => {
    ...
    return { attended, missed, upcoming, scheduled, rate }
      ...
      >
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {/* Mobile: title on row 1, 3-up button row on row 2 */}
            ...
            {/* Desktop: single row, title centred, buttons on either side */}
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-2 justify-center p-2 sm:p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
            ...
            <LegendChip color="bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600" label="No session" />
          </div>
          ...
          <p className="text-xs text-center text-gray-500 dark:text-gray-400">
            Attendance rate counts sessions up to today in {format(currentMonth, 'MMMM yyyy')}
            {monthStats.upcoming > 0 ? ` · ${monthStats.upcoming} upcoming` : ''}
            {monthStats.rate === null ? ' · no scheduled sessions yet' : ''}
          </p>
        </div>
      </Modal>
